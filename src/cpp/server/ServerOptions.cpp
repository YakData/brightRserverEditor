                            ...
                            std::ostream& osWarnings)
{
   ...
   server.add_options()
      ...
      ("server-add-header",
       value<std::vector<std::string>>(&serverAddHeaders_)->default_value(std::vector<std::string>{})->multitoken(),
         "adds a header to all responses from brightR Server");
   ...
   auth.add_options()
      ...
      ("auth-cookies-force-secure",
        ...
        "forces auth cookies to be marked as secure - should be enabled if running an SSL terminator infront of brightR Server");
