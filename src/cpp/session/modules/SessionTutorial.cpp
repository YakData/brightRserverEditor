                               ...
                               http::Response* pResponse)
{
   ...
   if (tutorialIndex().empty())
   {
      ...
      if (!module_context::isPackageInstalled("learnr"))
      {
         ...
         ss << "<p>The <code>learnr</code> package is required to run tutorials for the YakData Editor.</p>"
            << "<p>Please " << clickHere.str() << " to install the <code>learnr</code> package.</p>";
     }
     else
     {
        ss << "<p>Please wait while the Editor finishes indexing available tutorials...</p>";
     }
