                  ...
                  const std::string& type)
   {
      ...
      if (type == kTestShinyFile) {
        ...
        else
        {
           ...
           terminateWithError("Could not find brightR app for test in " +
      ...
      if (type == kTestShiny) {
        ...
        terminateWithError("brightR test type is unsupported.");
      }

      enqueCommandString("Testing brightR application using 'shinytest'");
      successMessage_ = "\nTest complete";
      module_context::processSupervisor().runCommand(cmd,
                                                     testOptions,
                                                     cb);
