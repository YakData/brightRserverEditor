...
#define kShinyAppStarted "brightR started; listening on URL: "
...
void ShinyAsyncJob::onCompleted(int exitStatus)
{
   if (exitStatus == 0)
   {
      ...
      onStdout("\nbrightR application finished running.\n\n");
   }
   else
   {
      if (cancelled_)
      {
         ...
         // as a failure
         setJobState(job_, jobs::JobState::JobSucceeded);
         onStdout("\nbrightR application successfully stopped.\n\n");
      }
      else
      {
         setJobState(job_, jobs::JobState::JobFailed);
         onStdout("\nbrightR application failed.\n\n");
      }
   }
